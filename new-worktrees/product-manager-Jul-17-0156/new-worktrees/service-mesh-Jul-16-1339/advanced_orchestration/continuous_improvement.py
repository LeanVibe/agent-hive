...
class ImprovementOpportunity:
    ...
    implementation_status: str = "identified"  # identified, planned, in_progress, completed, failed
        ...
        for bottleneck in bottlenecks:
            if bottleneck["type"] == "slow_task":
                opportunity = ImprovementOpportunity(
                    id=f"perf_bottleneck_{bottleneck['task_id']}",
                    type=ImprovementType.PERFORMANCE_OPTIMIZATION,
                    priority=ImprovementPriority.HIGH if bottleneck["average_time"] > 600 else ImprovementPriority.MEDIUM,
                    title=f"Optimize slow task: {bottleneck['task_id']}",
                    description=f"Task {bottleneck['task_id']} is consistently slow (avg: {bottleneck['average_time']:.1f}s)",
                    current_state={"average_time": bottleneck["average_time"]},
                    target_state={"average_time": bottleneck["average_time"] * 0.7},
                    expected_impact=0.8,
                    implementation_effort=0.6,
                    success_metrics=["task_completion_time", "system_throughput"],
        ...
        if agent_combinations:
            ...
            for combo, efficiency in low_efficiency_combos:
                opportunity = ImprovementOpportunity(
                    ...
                    success_metrics=["coordination_efficiency", "agent_utilization"],
                    implementation_steps=[
                        "Analyze agent interaction patterns",
                        "Identify coordination bottlenecks",
    ...
    def _identify_routing_opportunities(self, patterns: Dict[str, Any], coordination_data: List[Dict[str, Any]]) -> List[ImprovementOpportunity]:
        ...
        for failure_pattern in failure_patterns:
            if failure_pattern.get("type") == "routing_failure":
                opportunity = ImprovementOpportunity(
                    id=f"routing_failure_{failure_pattern['type']}",
                    type=ImprovementType.TASK_ROUTING,
                    priority=ImprovementPriority.HIGH,
                    title="Improve task routing to reduce failures",
                    description=f"Routing failures detected: {failure_pattern['frequency']} occurrences",
                    current_state={"failure_rate": failure_pattern["frequency"]},
                    target_state={"failure_rate": failure_pattern["frequency"] * 0.3},
                    expected_impact=0.8,
                    implementation_effort=0.7,
                    success_metrics=["routing_success_rate", "task_completion_rate"],
                    implementation_steps=[
                    ...
                    affected_agents=failure_pattern.get("affected_agents", []),
    ...
    def _identify_dependency_opportunities(self, patterns: Dict[str, Any], coordination_data: List[Dict[str, Any]]) -> List[ImprovementOpportunity]:
        ...
        if dependency_bottlenecks:
            opportunity = ImprovementOpportunity(
                ...
                description=f"Found {len(dependency_bottlenecks)} dependency-related bottlenecks",
                current_state={"dependency_bottlenecks": len(dependency_bottlenecks)},
                target_state={"dependency_bottlenecks": 0},
                expected_impact=0.7,
                implementation_effort=0.6,
                success_metrics=["dependency_resolution_time", "workflow_completion_time"],
                implementation_steps=[
    ...
    def _create_coordination_actions(self, opportunity: ImprovementOpportunity) -> List[ImprovementAction]:
        ...
        comm_action = ImprovementAction(
            ...
            parameters={
                ...
                "improvements": ["message_batching", "priority_queuing", "compression"]
            },
            target_agents=opportunity.affected_agents,
            execution_order=1
        )
    ...
    def _create_routing_actions(self, opportunity: ImprovementOpportunity) -> List[ImprovementAction]:
        ...
        routing_action = ImprovementAction(
            ...
            parameters={
                ...
                "fallback_strategies": ["round_robin", "least_loaded", "capability_based"],
