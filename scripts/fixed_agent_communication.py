...
def send_message_to_agent_fixed(
    ...
    for window_name in window_names_to_try:
        try:
            ...
            test_cmd = ["tmux", "list-windows", "-t",
                        session_name, "-F", "#{window_name}"]
    ...
    print(
        f"❌ Failed to send message to {agent_name} - no working window found")
    return False
...
def main():
    ...
    success = send_message_to_agent_fixed(
        args.agent, args.message, auto_submit)
