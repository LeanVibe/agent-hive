...
class PMMonitor:
    ...
    async def start_monitoring(self) -> None:
        ...
        self.logger.info("📋 Prevention-First Approach: Health checks and auto-respawn")
    ...
    async def _check_process_health(self, process_name: str, process_config: dict) -> None:
        ...
        try:
            ...
            if process_config.get("health_check"):
                health_method = getattr(self, process_config["health_check"], None)
                if health_method:
    ...
    async def _default_health_check(self, process_name: str, process_config: dict) -> bool:
    ...
    async def cli_health_check(self, process_name: str, process_config: dict) -> bool:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.error(f"CLI health check failed: {stderr.decode()}")
                return False
    ...
    async def quality_gates_health_check(self, process_name: str, process_config: dict) -> bool:
    ...
    async def _handle_unhealthy_process(self, process_name: str, process_config: dict) -> None:
        """Handle unhealthy process detection."""
        if not process_config.get("auto_restart", False):
            self.logger.info(f"⚠️  {process_name} is unhealthy but auto-restart is disabled")
            return
        ...
        if process_info["restart_attempts"] >= self.config["max_restart_attempts"]:
            self.logger.error(f"❌ {process_name} exceeded max restart attempts")
    ...
    async def _restart_process(self, process_name: str, process_config: dict) -> None:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                self.logger.error(f"❌ Failed to restart {process_name}: {stderr.decode()}")
    ...
    def _generate_health_report(self) -> None:
        ...
        healthy_count = sum(1 for status in self.health_checks.values() if status)
        total_count = len(self.health_checks)
        
        self.logger.info(f"📊 Health Report: {healthy_count}/{total_count} processes healthy")
    ...
    async def _check_critical_alerts(self) -> None:
        ...
        if unhealthy_critical:
            alert_message = f"🚨 Critical processes unhealthy: {', '.join(unhealthy_critical)}"
