...
class MetricsReviewCommand(ReviewCommand):
    ...
    def _is_metric_healthy(self, metric: str, value: Union[int, float]) -> bool:
...
class QualityGateReviewCommand(ReviewCommand):
    ...
    def _display_gate_status(self, gate_name: str, gate_data: Dict[str, Any]) -> None:
...
class ReviewOrchestrator:
    ...
    async def execute_review(self, review_type: str, **kwargs) -> None:
        ...
        filtered_kwargs = self._filter_kwargs_for_command(review_type, **kwargs)
        await command.execute(**filtered_kwargs)
    
    def _filter_kwargs_for_command(self, review_type: str, **kwargs) -> Dict[str, Any]:
